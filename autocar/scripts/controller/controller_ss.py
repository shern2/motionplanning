...
class NewtonController:
    ...
    def update_velX(self):
        self.velX = self.velX + self.yaw * 1.0 / self.hz
        if self.velX > self.max_speed:
            self.velX = self.max_speed
        if self.velX < self.min_speed:
            self.velX = self.min_speed
        return self.velX

    ## Assumes controls are (acc, theta). Computes velX and velY accordingly
    def update_velXY(self):
        self.vel = self.vel + self.acc * 1.0 / self.hz
        if self.vel > self.max_speed:
            self.vel = self.max_speed
        if self.vel < self.min_speed:
        ...
        self.velY = self.vel * cos(radians(self.yaw))
        self.velX = self.vel * sin(radians(self.yaw))
    ...
    def run(self):
        while not rospy.is_shutdown():
            if self.start_game:
                vel = self.update_vel()
                yaw = self.update_velX()
                self.send_control(vel, yaw)

                # velX, velY = self.update_velXY()
                # self.send_control(velY,velX)                
